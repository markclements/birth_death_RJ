MCMC.BD<-function(phy,div,ngen=1000000,sfreq=100,root.rate=0.01,root.rate.e=0,e=3,lam.d=1,lam.e=0.5,lamsplit.e=0.5,msprob.d=0.25,msprob.e=0.25,rateprob.d=0.25,rateprob.e=0.25,rootprob.d=0.5,rootprob.e=0.5,internal.only=FALSE,assign.d=FALSE,assign.e=FALSE,directory="~/Desktop/trials"){
	require(ape)
### Prepare data	
...
# initiallize chain
	...
	edge<-rate.calc(edge,rate.vect,assing.d)
	edge<-rate.calc.e(edge,rext.vect,assing.e)
	lnLik<-Lnlikelihood(edge)
# Proposal Counts
...
# BEGIN CALCULATION
	...
	for(i in (1:ngen)) {
	  	...
	  	z<-sample(1:4,1,prob=c(msprob.d,msprob.e,rateprob.d,rateprob.e)  	
			 if (z==1){					#adjust number of d rates
				...
				rext.vect.new=rext.vect
			  	...
			  	xx<-mergeORsplit.e(edge,rext.vect,e,internal.only,assign.e,lamsplit.e)
			  	newedge<-xx$newedge
	  	...
	  	newlnLik<-Lnlikelihood(newedge)
	  	lr <- newlnLik-lnLik
		ratio <- exp(lr)*HP
		
		if (runif(1) <= ratio) {			## adopt proposal ##
			decision="adopt"
			lnLik<-newlnLik
			edge<-new.edge
